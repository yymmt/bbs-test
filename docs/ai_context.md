...
- 方針の相談の時:
  ...
  - 相談内容に応じて、取りうる技術的な手段のバリエーションを提示したり、ソースコードの修正方針などを提示してください。（その場合も直ちにソースコードを修正するのではなく、下記「仕様変更の指示」「コード反映の指示」「微調整の指示」を待ってください。）
- 仕様変更の指示時:
...
- コード反映の指示時:
  ...
  - リファクタリングは積極的に行わないが、リファクタリングが強く推奨される場合は報告して下さい。(コードをすぐに修正するのではなく。)
- 微調整の指示時:
  - 指定したコード内に「微調整指示」とコメントがある場合、内容に応じて修正してください。リファクタリングやレイアウト修正を想定しています。指示が現実的でない、あるいは大きくデメリットがある場合は報告してください。
- 共通ルール:
...
- クライアント側もAngular等のフレームワークやjQueryなどのライブラリを使用しない（CSS/Font/QRコード等の特定用途のライブラリは除く）。
...
- 認証方式: LocalStorageにUUID (v4) を保存し、リクエストヘッダー (X-USER-ID) で送信することでユーザーを識別する（簡易認証）。
  - 機種変更等に対応するため、引き継ぎコード発行・入力機能を実装する。
...
- 画面構成: SPA構成。ヘッダーには、戻るボタン（左上）、ページタイトル（中央）、ハンバーガーメニュー（右上）を配置。
  ...
  - ハンバーガーメニューで以下の画面を切り替える。今開いている画面によって、メニュー自体の表示・非表示も制御する。
    - スレッド一覧画面（ホーム）
    - ユーザー設定画面
    - スレッド設定画面
    - チェックリスト一覧画面
...
- 初期設定 (Welcome画面): 初回アクセス時（UUID未保持）に表示。
  ...
  - 多言語対応: 日本語と英語の切り替えに対応する。
    - 画面上のテキストやプレースホルダーを言語設定に応じて切り替える。
    - 言語切り替え時はリロードなしで即時反映する。
  - インストール促進: ブラウザでアクセス時、ホーム画面への追加を促すUIを表示する。
    - Android/PC等: 「アプリのインストール」ボタンを表示し、クリックでインストールプロンプトを呼び出す。
    - iOS: ブラウザの共有メニューからの追加方法をガイド表示する。
    - PWA（スタンドアロン）モード時は表示しない。
...
- ユーザー設定: 自身の名前変更、言語切り替え、引き継ぎコード発行など。ヘッダーのページタイトルに「User Settings」と表示する。アプリのバージョン情報を表示する。
...
- 通知機能: スレッドに新着投稿があった際、参加している他のユーザーへWeb Push通知を送信する。
  ...
  - アプリ使用中（アクティブ）の挙動:
    - 対象スレッドを開いている場合: 通知は表示せず、画面を自動更新する（新規投稿の表示、削除の反映）。
      - ※判定ロジック: Service Workerからクライアントへ問い合わせメッセージを送信し、クライアントが保持する現在のスレッドIDと照合して判定する（URL判定は信頼性が低いため使用しない）。
    - 対象スレッド以外を開いている場合: 通常通り通知を表示する。
  - 削除時の挙動:
    - 対象スレッドを開いている場合: 画面から該当投稿を削除する。
    - それ以外の場合: 通知は表示しない。
    - 将来対応: 削除時に、画面が一番下へスクロールするが、スクロール位置を保持するようにしたい。
- PWA対応: スマートフォンのホーム画面に追加可能にする（manifest.json, Service Worker）。

# 管理ツール仕様 (詳細設計)
- /admin/index.php: deploy.php, migrate.php へのリンクを表示。GitHubリンク（比較）を表示する。git pullやマイグレーションが必要か判定し、不要な場合はリンクを無効化する。
- /admin/deploy.php: `git pull` を実行する。
- /admin/migrate.php: マイグレーションを実行する。
...
- POST /api.php
  ...
  - action=generate_transfer_code: 引き継ぎコードを発行して返す。有効期限を設定する。
